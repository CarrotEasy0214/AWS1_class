...
// rootPath를 상수로 선언하고 path 객체에서 join 매서드를 호출해 그 반환값을 할당한다.

const find = (currPath) => {
  // 1. /mnt/c/Users/AM/Documents/카카오톡 받은 파일/Desktop (2)/AWS-1/9.NodeJS/240325/views
  // 2. /mnt/c/Users/AM/Documents/카카오톡 받은 파일/Desktop (2)/AWS-1/9.NodeJS/240325/views/imgs
  ...
  // "find 함수 첫 호출" 때에는 rootPath를 전달 받아 currpath에 할당했다.

  console.log("directory", directory);
  //find함수 첫 호출 [ 'index.html', 'index.js', 'user', 'write', 'write1' ]
  // 2. png파일
  directory.forEach((currPathName) => {
    // 1-1. 'imgs'가 currPathName으로 할당됨
    // 2. png파일이 currPathName으로 할당됨
    // 1-2. 'index.html'가 currPathName으로 할당됨
    const findPath = path.join(currPath, currPathName);
    // 상수 findPath를 선언하고 path 객체에서 join 메서드를 호출하여 그 반환값을 할당
    // 1-1. rootPath 한테 imgs 를 붙임
    // 2. /mnt/c/Users/AM/Documents/카카오톡 받은 파일/Desktop (2)/AWS-1/9.NodeJS/240325/views/imgs/png
    // 1-2. /mnt/c/Users/AM/Documents/카카오톡 받은 파일/Desktop (2)/AWS-1/9.NodeJS/240325/views/index.html
    console.log("statSync", fs.statSync(findPath));
    const isFile = fs.statSync(findPath).isFile();
    // fs 객체의 stateSync 메서드를 호출하여 반환 받는다. //s.statSync(findPath)
    // 반환 받은 것의 타입 => 객체
    // 반환 받은 객체에서 isFile을 찾아서 호출한다. => 호출(`()`)로 fs.statSync(findPath)가 객체임을 알수있다.
    ...
    if (isFile) {
      ...
      // statatic 객체의 findPath.slice(rootPath.length 이름으로 findPath를 값으로 할당한다.
      // 프로퍼티가 추가된다.
      ...
      find(findPath);
      // 1. imgs는 폴더이기 때문에 isFile == false , find 함수를 호출한다. => 이후로 숫자는 2가 된다.
      // 1,2 등 붙는 숫자는 뭘까? 실행 컨텍스트가 몇개가 쌓였나 붙는 숫자
